...
<template>
  <div
    class="fixed inset-0 z-[100] flex flex-col items-center justify-center overflow-hidden bg-[#05070a]"
  >
    ...
    <div class="absolute inset-0 z-0">
      <div
        v-for="n in 50"
        :key="n"
        class="absolute animate-pulse rounded-full bg-white opacity-20"
        :style="{
          top: Math.random() * 100 + '%',
    ...
    <!-- Atmospheric Glow -->
    <div
      class="bg-primary/10 pointer-events-none absolute bottom-0 left-1/2 h-[400px] w-[800px] -translate-x-1/2 rounded-full blur-[120px]"
    ></div>

    <!-- Main Content Container with Vibration -->
    <div class="animate-lift-off relative z-10 flex flex-col items-center">
      <!-- Artemis II (SLS) Rocket SVG -->
      <div class="rocket-container group relative">
        <svg viewBox="0 0 100 240" class="w-32 drop-shadow-[0_0_30px_rgba(252,61,33,0.3)] md:w-48">
          <!-- Launch Abort System (LAS) -->
          <path d="M50 5 L50 25" stroke="#dfe2ee" stroke-width="1" />
          <path d="M48 25 L52 25 L50 15 Z" fill="#dfe2ee" />

          <!-- Orion Capsule & ESM -->
          <path d="M46 25 L54 25 L56 35 L44 35 Z" fill="#8e94a5" />
          <!-- Orion -->
          <rect x="44" y="35" width="12" height="15" fill="#5c6374" />
          <!-- Service Module -->

          <!-- SLS Core Stage (Orange Tank) -->
          <path d="M42 50 L58 50 L58 200 L42 200 Z" fill="#D35400" />
          <!-- Main Body -->
          <rect x="42" y="190" width="16" height="10" fill="#BA4A00" />
          <!-- Base darker section -->
          ...
          <path d="M34 85 Q37 75 40 85 Z" fill="#FFFFFF" />
          <!-- Tip -->
          <rect x="34" y="100" width="6" height="2" fill="#2C3E50" />
          <!-- Mark -->
          <rect x="34" y="170" width="6" height="2" fill="#2C3E50" />
          <!-- Mark -->

          <!-- Right Booster -->
          <path d="M60 85 L66 85 L66 190 L60 190 Z" fill="#FFFFFF" />
          <path d="M60 85 Q63 75 66 85 Z" fill="#FFFFFF" />
          <!-- Tip -->
          <rect x="60" y="100" width="6" height="2" fill="#2C3E50" />
          <!-- Mark -->
          <rect x="60" y="170" width="6" height="2" fill="#2C3E50" />
          <!-- Mark -->
      ...
      <div class="mt-8 flex flex-col items-center">
        <h3 class="font-headline text-on-surface/80 text-xs font-black uppercase tracking-[0.6em]">
          INITIALIZING ORBITAL LINK...
        </h3>

        <!-- Progress Bar -->
        <div class="mt-4 h-[2px] w-48 overflow-hidden rounded-full bg-white/10">
          <div class="bg-primary animate-progress h-full"></div>
        </div>

        <p
          class="font-label text-on-surface-variant mt-4 animate-pulse text-[8px] font-bold uppercase tracking-[0.4em]"
        >
    ...
    <div class="absolute bottom-12 right-12 scale-75 opacity-5 grayscale invert">
...
@keyframes shake {
  ...
  20% {
    transform: translate(-0.5px, 0px) rotate(0.05deg);
  }
  40% {
    transform: translate(0.5px, -0.5px) rotate(0deg);
  }
